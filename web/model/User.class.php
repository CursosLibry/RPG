...
class User extends Model{
    ...
    protected $password;
  	...
  	public function insert(){ // da insert nos dados que o usuário irá cadastrar inicialmente
        try{
            $c = $this->conn->prepare("INSERT INTO user(name,email,gender,nationality,race,status,nick,password) 
    		VALUES(:name,:email,:gender,:nationality,:race,:status,:nick,:password)");
...
// para o futuro tratamento de erros
    ...
    public function newInsert(){ // da insert nos dados que o usuário irá cadastrar após seu cadastro
        try{
            $c = $this->conn->prepare("INSERT INTO user(name,email,gender,nationality,race,status,nick,password) 
    		VALUES(:name,:email,:gender,:nationality,:race,:status,:nick,:password)");
    		$c->bindValue(':name',$this->name,PDO::PARAM_STR);
